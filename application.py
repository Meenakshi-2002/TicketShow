...
def search():
    ...
    if cat == 'venue':
        venues = Venue.query.filter(Venue.place.contains(q)).all()
        show_all = Show.query.all()
        return render_template('index.html',show_all=show_all, search_venues=venues, cat=cat, q=q)
    elif cat=='show':
        shows = Show.query.filter(Show.show_name.contains(q)).all()
        show_all = Show.query.all()
        return render_template('index.html', search_shows=shows, show_all=show_all, cat=cat, q=q)
...
def dashboard():
    ...
    shows = Show.query.all()
    # tickets = Ticket.query.all()
    return render_template('dashboard.html', users=users ,venues=venues, shows=shows)
